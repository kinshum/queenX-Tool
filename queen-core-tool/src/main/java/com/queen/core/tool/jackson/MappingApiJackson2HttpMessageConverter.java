...
public class MappingApiJackson2HttpMessageConverter extends AbstractReadWriteJackson2HttpMessageConverter {
	...
	public MappingApiJackson2HttpMessageConverter(ObjectMapper objectMapper, QueenJacksonProperties properties) {
		super(objectMapper, initWriteObjectMapper(objectMapper, properties), initMediaType(properties));
	}

	private static List<MediaType> initMediaType(QueenJacksonProperties properties) {
	...
	private static ObjectMapper initWriteObjectMapper(ObjectMapper readObjectMapper, QueenJacksonProperties properties) {
		// 拷贝 readObjectMapper
		ObjectMapper writeObjectMapper = readObjectMapper.copy();
		// 大数字 转 字符串
		if (Boolean.TRUE.equals(properties.getBigNumToString())) {
			writeObjectMapper.registerModules(QueenNumberModule.INSTANCE);
		}
		// null 处理
