...
public class SqlLogInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (DRUID_POOLED_PREPARED_STATEMENT.equals(stmtClassName)) {
		...
		} else if (stmtClassName.contains(DM_PREPARED_STATEMENT)) {//增加dm数据库的SQL输出功能
			Class<?> clazz = Class.forName(stmtClassName);
			//sql
			Field feild;
			String sql = null;
			Object params = null;
			try {
				feild = clazz.getSuperclass().getDeclaredField("originalSql");
				feild.setAccessible(true);
				sql = (String) feild.get(statement);
			} catch (Exception e) {
				e.printStackTrace();
			}
			//params
			try {
				feild = clazz.getDeclaredField("params");
				feild.setAccessible(true);
				params = feild.get(statement);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (sql != null) {
				originalSql = sql + "\t  params: " + (params == null ? "[]" : params);
			}
		}
		if (originalSql == null) {
